...
private:
  ...
  edm::EDGetTokenT <CSCComparatorDigiCollection>  m_comparatorDigiToken;
...
CSCChamberMasker::CSCChamberMasker(const edm::ParameterSet& iConfig) 
  ...
  ,m_comparatorDigiToken(consumes<CSCComparatorDigiCollection>(iConfig.getParameter<edm::InputTag>("comparatorDigiTag")) )
{
...
CSCChamberMasker::produce(edm::Event& event, const edm::EventSetup& conditions)
{
  ...
  std::unique_ptr<CSCComparatorDigiCollection> filteredComparatorDigis(new CSCComparatorDigiCollection());
  ...
  ageDigis<CSCComparatorDigi>(event,m_comparatorDigiToken,randGen,filteredComparatorDigis);
  ...
  if(!digiToken.isUninitialized())
      ...
      for ( const auto & j : (*digis) ) 
	{
		...
		if (((std::is_same<T,CSCStripDigi>::value && type == EFF_WIRES)  || 
		     ...
		     (std::is_same<T,CSCComparatorDigi>::value && type  == EFF_COMPARATORS) || 
